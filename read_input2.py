...
def read_ressources(v, ville):
    ...
    wookbook = openpyxl.load_workbook(
        "InstancesV"+str(v)+"/Instance"+ville+"V"+str(v)+".xlsx".format(ville))
    ...
    for row in worksheet.iter_rows(2, max_row=INF):
        ...
        ressources[row[0].value] = Ressource(
            row[1].value, row[2].value, row[3].value, row[4].value, time1, time2)
    return ressources
...
def read_ressources_unavailabilities(v, ville, ressources):
    ...
    # Define variable to load the wookbook
    wookbook = openpyxl.load_workbook(
        "InstancesV"+str(v)+"/Instance"+ville+"V"+str(v)+".xlsx")
    ...
    for row in worksheet.iter_rows(2, max_row=INF):
        ...
        ressources_unavailabilities[row[0].value] = RessourceUnavailability(
            row[1].value, row[2].value, time1, time2)
        ressources[row[0].value].addUnavailability(
            ressources_unavailabilities[row[0].value])
...
def read_tasks(v, ville):
    ...
    for row in worksheet.iter_rows(2, max_row=INF):
        ...
        tasks[row[0].value] = Task(
            row[1].value, row[2].value, row[3].value, row[4].value, row[5].value, time1, time2)
    return tasks
...
def read_tasks_unavailabilities(v, ville, tasks):
    ...
    for row in worksheet.iter_rows(2, max_row=INF):
        ...
        tasks[row[0].value].addUnavailability(
            tasks_unavailabilities[row[0].value])
    return tasks_unavailabilities


def read_input(v, ville):
    r, t = read_ressources(v, ville), read_tasks(v, ville)
    ru, tu = read_ressources_unavailabilities(
        v, ville, r), read_tasks_unavailabilities(v, ville, t)
    return r, ru, t, tu
